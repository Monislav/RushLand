...
public class SPatrol : BaseFSM {
    GameObject[] waypoints;
    int i;

    public void Awake()
    {
        waypoints = GameObject.FindGameObjectsWithTag("SWaypoint").OrderBy(x => x.name).ToArray();
    }
    ...
    override public void OnStateUpdate(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
    {
        if (waypoints.Length == 0)
            return;
        if (Vector3.Distance(waypoints[i].transform.position, npc.transform.position) < 20)
        {
            i++;
        }
